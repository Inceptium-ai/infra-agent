...
4. [Investigating Issues](#investigating-issues)
5. [Running Audits](#running-audits)
6. [Making Infrastructure Changes](#making-infrastructure-changes)
7. [Creating New Infrastructure](#creating-new-infrastructure)
8. [Querying Infrastructure](#querying-infrastructure)
9. [AWS and Git Queries](#aws-and-git-queries)
10. [Pipeline Artifacts](#pipeline-artifacts)
...
The chat interface provides special commands (prefixed with `/`) for session management:

| Command | Description |
|---------|-------------|
| `/status` | Show status of all agent tasks (running/completed/failed) |
...
Use `/status` to see all agent tasks in the current session:
...
$ /status
                      ...
                      Agent Tasks
┏━━━━━━━━━━┳━━━━━━━┳━━━━━━━━━━━┳━━━━━━━━━┳━━━━━━━━━━━━━━━━┓
┃ ID       ┃ Agent ┃ Status    ┃ Started ┃ Current Step   ┃
┡━━━━━━━━━━╇━━━━━━━╇━━━━━━━━━━━╇━━━━━━━━━╇━━━━━━━━━━━━━━━━┩
│ task-a1b2│ chat  │ COMPLETED │ 45s ago │ -              │
│ task-c3d4│ chat  │ RUNNING   │ 2s ago  │ Calling aws... │
└──────────┴───────┴───────────┴─────────┴────────────────┘
```
...
The agent can query AWS resources and Git repositories directly using MCP (Model Context Protocol) tools.

### AWS Queries

Query any AWS service using natural language. The agent translates your request to the appropriate boto3 API call.
...
**You:** `List all running EC2 instances`

**Agent:**
```
**EC2 Instances (Running):**

| Instance ID         | Type      | State   | Name              | AZ          |
|---------------------|-----------|---------|-------------------|-------------|
| i-02c424847cd5f557e | t3.small  | running | infra-agent-dev-bastion | us-east-1a |
| i-0a1b2c3d4e5f6g7h8 | t3.medium | running | infra-agent-dev-node-1  | us-east-1a |
| i-0b2c3d4e5f6g7h8i9 | t3.medium | running | infra-agent-dev-node-2  | us-east-1b |
| i-0c3d4e5f6g7h8i9j0 | t3.medium | running | infra-agent-dev-node-3  | us-east-1c |

Total: 4 running instances
```

### Git Queries (Drift Detection)

Query IaC files in GitHub/GitLab repositories and compare with deployed state.

**Prerequisites:**
- `GITHUB_TOKEN` or `GH_TOKEN` in `.env` file for GitHub access
- `GITLAB_TOKEN` in `.env` file for GitLab access

**Example Prompts:**

```
List IaC files in the infra-agent repository
```
```
Read the VPC CloudFormation template from Git
```
```
Check for drift between Git and AWS
```
```
Compare deployed CloudFormation stacks with Git
```

**Example Session - Drift Detection:**

**You:** `Check for drift between Git and AWS for the infra-agent project`

**Agent:**
```
**IaC Drift Analysis**

Comparing: GitHub Inceptium-ai/infra-agent vs AWS Account 340752837296

**CloudFormation Stacks:**

| Stack Name              | Git Template | AWS Status | Drift     |
|-------------------------|--------------|------------|-----------|
| infra-agent-dev-vpc     | ✓            | DEPLOYED   | None      |
| infra-agent-dev-bastion | ✓            | DEPLOYED   | None      |
| infra-agent-dev-cluster | ✓            | DEPLOYED   | IN_SYNC   |
| infra-agent-dev-cognito | ✓            | DEPLOYED   | None      |
| gitlab-storage          | ✓            | NOT_FOUND  | Missing   |

**Findings:**
- [INFO] gitlab-storage.yaml exists in Git but stack not deployed
- [OK] All deployed stacks match their Git templates

**Recommendation:**
- Review gitlab-storage.yaml - either deploy it or remove from Git
```

### Supported AWS Services

The agent can query ANY AWS service via boto3, including:
...
| **Management** | CloudFormation, SSM, Config |
...
| 1.1 | 2026-01-19 | AI Agent | Added Chat Commands section (progress feedback, /status); Added AWS and Git Queries section (MCP integration) |
